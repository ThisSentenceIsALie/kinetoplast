...
#Class defining the tkinetoplast object. This should let you have multiple kinetoplasts simulants running around at once. Maybe one day they'll even interact with eachother.
class tkinetoplast:
    ...
    def __init__(self,xdim,ydim):
        self.xdim=xdim
        self.ydim=ydim
        self.grid,self.spacing=gg.triangular_grid(ydim, xdim)
        self.kpg=list()
        self.kpn=''
        self.idm=list()
        self.maxi=False
    ...
    def add_maxi_circles(self, maxi_rad, amount):
        ...
        self.amount=amount
        self.maxi=True
        self.grid=gg.add_maxi_circles(self.grid,(self.xdim+self.spacing/2+0.5),(self.ydim*sqrt(3)/2+0.5),maxi_rad,amount,hexg=True)

    #Method to compile the current grid into an adjacency matrix and networkx graph object
    def compile_graph(self):
        self.kpg, idm =gg.to_graph_triangular(self.grid,self.spacing)
