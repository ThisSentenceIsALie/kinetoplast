...
#Class defining the hkinetoplast object. This should let you have multiple kinetoplasts simulants running around at once. Maybe one day they'll even interact with eachother.
class hkinetoplast:
    ...
    def __init__(self,xdim,ydim,flag_bound=False,rad=0.5,sep=1,file_location='C:\\Users\\natha\\OneDrive\\Desktop\\CSULB\\kinetoplast\\'):
        self.xdim=xdim
        self.ydim=ydim
        self.rad=rad
        self.sep=sep
        self.grid=gg.hexagonal_grid(sep, ydim, xdim, rad,flag_bound)
        self.kpg=list()
        self.kpn=''
        self.idm=list()
        self.maxi=False
    ...
    def add_boundary(self,saturation):
        self.supersat=True
        self.grid=gg.hexagonal_boundary(self.grid,saturation)
        self.sat+=saturation
    ...
    def add_maxi_circles(self, maxi_rad, amount):
        ...
        self.amount=amount
        self.maxi=True
        self.grid=gg.add_maxi_circles(self.grid,self.grid[0][(len(self.grid[0])-1)][0][0],self.grid[(len(self.grid)-1)][0][0][1],maxi_rad,amount,hexg=True)

    #Method to compile the current grid into an adjacency matrix and networkx graph object
    def compile_graph(self):
        self.kpg, self.idm =gg.to_graph_hex(self.grid,self.sep)
        self.kpn=gf.to_network(self.kpg, self.grid,self.idm,self.flag_bound)
