...
class hkinetoplast3D:
    ...
    def __init__(self,xdim,ydim,zdim):
        ...
        self.ydim=ydim
        self.zdim=zdim
        self.grid=gg.graphite_grid(xdim,ydim,zdim)
        self.kpg=list()
        self.kpn=''
        self.idm=list()
        self.maxi=False
    ...
    def add_boundary(self,saturation):
        self.supersat=True
        self.grid=gg.graphite_boundary(self.grid,saturation)
        self.sat+=saturation

    #Method to sprinkle maxi circles of a given radius and number throughout the grid
    def add_maxi_circles(self, maxi_rad, maxi_amount):
        self.maxi_rad=maxi_rad
        self.amount=amount
        self.maxi=True
        self.grid=gg.graphite_maxi_circles(self.grid,maxi_rad,maxi_amount,self.xdim,self.ydim,self.zdim)

    #Method to compile the current grid into an adjacency matrix and networkx graph object
    def compile_graph(self):
        self.kpg, idm =gg.to_graph_graphite(self.grid)
